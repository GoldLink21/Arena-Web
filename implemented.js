...
class Entity{
    ...
    equipWeapon(wep) {
        ...
        if(wep instanceof Weapon) {
            ...
            this.weapon = new Weapon(wep, this);
        }
    }
}

function drawImgRotated(img,deg){
...
var pw={
    ...
    ball:weapon('spear', 10, 10, 'red', 3, 1, 2)
}
...
class Weapon extends Entity{
    ...
    swing(){
        if(this.canSwing&&!this.active&&!stats.isPaused){
            ...
            //This sets this.dir to a number
            this.dir=Angle.degToCardinal(this.parent.dir.deg)
            switch(this.dir){
    ...
    move(){
        if(this.canMove){
            if(this.buttonDirs.up){
                this.moveIn(Angle.up)
                if(this.checkTile())
                   this.moveIn(Angle.down)
            }if(this.buttonDirs.down){
                this.moveIn(Angle.down)
                if(this.checkTile())
                    this.moveIn(Angle.up)
            }if(this.buttonDirs.left){
                this.moveIn(Angle.left)
                if(this.checkTile())
                    this.moveIn(Angle.right)
...
function pMoveHelp(event,bool){
    switch(event.key){
        case 'w':case'ArrowUp':
            if(bool)player.dir=Angle.up
            player.buttonDirs.up=bool;
            break;
        case 's':case'ArrowDown':
            if(bool)player.dir=Angle.down
            player.buttonDirs.down=bool
            break;
        case 'a':case'ArrowLeft':
            if(bool)player.dir=Angle.left
            player.buttonDirs.left=bool
            break;
        case 'd':case'ArrowRight':
            if(bool)player.dir=Angle.right
...
function floor(){
    var nFloors=3
    switch(Random.intTo(nFloors)/*Math.floor(Math.random()*nFloors)*/){
        case 0:
...
function rnd(start,end){
    if(Array.isArray(start))
        return Random.arrayElement(start)
    if(typeof start==='object')
        return Random.objProperty(start)
    return Random.intRange(start,end)
}
...
function randomWaves(){
    ...
    if(entities.length>=maxEnemies){}//This catches if there is too many enemies before the wave increments
        ...
        addEnemy(rnd(et),rnd(ew),rnd(2,4));
    }else
...
/**@example rndObjNot(ew,[ew.boss,ew.small]) */
function rndObjNot(obj,/**@type {any[]} */notArr){
    var ele;
    do{
        ele=Random.objProperty(obj)
    }while(notArr.includes(ele))
    return ele;
}

function rndEnemy(){
    var type=rndObjNot(et,[et.boss])
    var wep;
    switch(type){
        case et.small:
            wep=rndObjNot(ew,[ew.boss,ew.bSpear])
            break;
        case et.basic:
            wep=rndObjNot(ew,[ew.boss])
            break
        case et.big:
            wep=rndObjNot(ew,[ew.boss])
            break
...
var draw=setInterval(drawAll,1000/60)
...
function resumeMove(){
    move=setInterval(moveAll,1000/30)
}
